...
import com.gl.education.composition.interactive.RAWFullScoreColumnInteractive;
...
public class RAWFullScoreColumnActivity extends BaseActivity {
    ...
    protected int provideContentViewId() {
        return R.layout.activity_rawfull_score_column;
    }
    ...
    public void initView() {
        ...
        top_title.setText("满分作文");

        String token = SPUtils.getInstance().getString(AppConstant.SP_TOKEN);
        token = "?token="+token;
        String url = AppConstant.YY_WEB_COMPOSITION_RAW_FULLSCORECOLUMN + token;
        ...
        mAgentWeb.getJsInterfaceHolder().addJavaObject("android", new RAWFullScoreColumnInteractive(mAgentWeb,
                this));
    }
    ...
    public void getFullScoreColumn(JSgetFullScoreColumnEvent event) {
        HomeAPI.getCatalog("3", new JsonCallback<GetHotColumnBean>() {
            @Override
            public void onSuccess(Response<GetHotColumnBean> response) {
                if (response.body().getResult() == 1000){
                    ...
                    mAgentWeb.getJsAccessEntrace().quickCallJs(AppConstant
                            .callJs_setFullScoreColumn,  json);
                }
            }
        });
    }
    ...
    //
    @Subscribe(threadMode = ThreadMode.MAIN)
    public void openList(JSFULLopenListeEvent event) {
