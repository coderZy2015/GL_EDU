package com.gl.education.payInfo;
...
public class MyWXPayUtils {
    ...
    private final static int PAY_CANCLE = 0;
    private final static int PAY_FINISH = 1;

    private IWXAPI iwxapi; //微信支付api

    private WXPayBuilder builder;

    private static String eventTag;     //eventbus的标记，确定传给哪个类
    private static String orderId;      //订单系统的账号

    private MyWXPayUtils(WXPayBuilder builder, String _tag, String id) {
    ...
    public static Handler wxHandler = new Handler() {
        public void handleMessage(Message msg) {
            ...
            if (msg.arg1 == PAY_FINISH){
                PayFinishEvent event = new PayFinishEvent();
                event.setMessage(eventTag);
                event.setOrderId(orderId);
                event.setResult(PAY_FINISH);
                EventBus.getDefault().post(event);
            }else if (msg.arg1 == PAY_CANCLE){
                PayFinishEvent event = new PayFinishEvent();
                event.setMessage(eventTag);
                event.setOrderId(orderId);
                event.setResult(PAY_CANCLE);
                EventBus.getDefault().post(event);
            }
    ...
    public static class WXPayBuilder {
        ...
        public MyWXPayUtils build(String tag, String id) {
            return new MyWXPayUtils(this, tag, id);
        }
