package com.gl.education.helper.rxjavahelper;
...
public class RxResultHelper {
    ...
    private static final int RESPONSE_SUCCESS_CODE = 1000; //大部分为200
                ...
                return tObservable.flatMap(
                        new Function<ResponseData<T>, Observable<T>>() {
                            ...
                            public Observable<T> apply(ResponseData<T> tResponseData) throws Exception {
                                //可以相应更改
                                if (tResponseData.getResult() == RESPONSE_SUCCESS_CODE) {
                                    return Observable.just(tResponseData.getData());
                                } else if (tResponseData.getResult() == RESPONSE_ERROR_CODE) {
